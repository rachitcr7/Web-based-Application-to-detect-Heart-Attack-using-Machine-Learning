...
def predict():
    ...
    #fitting(training) the data into Naive Bayes classifier
    ...
    if request.method == 'POST':
        # Input
        y = []
        y.append(int(request.form['age'][0]))
        y.append(int(request.form['gender'][0]))
        y.append(int(request.form['cp'][0]))
        y.append(int(request.form['bp'][0]))
        y.append(int(request.form['cholestrol'][0]))
        y.append(int(request.form['fasting_bp'][0]))
        y.append(int(request.form['ecg'][0]))
        y.append(int(request.form['hr'][0]))
        y.append(int(request.form['angina'][0]))
        y.append(float(request.form['op'][0]))
        y.append(int(request.form['slope'][0]))
        y.append(int(request.form['ca'][0]))
        y.append(int(request.form['thal'][0]))
        print(y)
        y = np.array(y)
        y = y.reshape(1, -1)
        ...
        # Predict        
        z_pred = str(nb.predict(y)[0])
        
    if (z_pred == '1'):
        return render_template('result.html',prediction = "High Risk")
    else:
